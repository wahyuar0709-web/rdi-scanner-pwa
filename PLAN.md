...
| Version | 1.29 |
| Date | 2026-09-27 |
| Repo HEAD | `66866e5` (Tahap 2 — harness eksekutabel Code.gs + 4 suite backend) |
| App version | `v15.28` / SW `rdi-stok-v36` / GAS deployment `@41` |
...
|------|-------|--------------|
| Git HEAD | `66866e5` (Tahap 2 — harness eksekutabel Code.gs + 4 suite backend) | — |
| Working tree | clean | — |
| `APP_VERSION` | `v15.28` (source of truth + title/apple/css/js/topbar sinkron) | L1 PASS |
| `APP_BUILD_DATE` | `2026-09-26` | L1 |
| SW `CACHE` | `rdi-stok-v36` (`sw.js:1`) + precache `./js/util.js` + `./js/cetak.js` + `./js/outbox.js` + `./js/format.js` + `./js/jspdf.min.js` | L1+L2 |
| `index.html` lines | ~4316 (Revisi Cetak: shell flex `#section-cetak` + bar in-flow; audit-tampilan: CSS alert scoped `@media 640` + `#modal-filter`/`#modal-sort` pindah body-level; F-06f: h1 login + ::after 44px + hash deep link + prefers-color-scheme; F-06e: 4 fallback `100dvh` + `role="dialog"` ×2; BUG-01: +8 export; fix-3-bug cetak: `#section-cetak .subpage-bar` in-flow + `<script jspdf.min.js>` + `generateOutput` label-download tanpa popup; fix-2-bug user: `#section-cetak .container{width:100%}` + blok sesi `SESSION_IDLE_MS`/`enforceSessionIdle`/dual-write) | L1 |
| Suite L1 in-repo | `tests/l1/` **34** suite (30 FE + **4 backend eksekutabel `gs_*`**) · `npm run test:l1` · assertion floor aktif | **L1 PASS 844/0** (2026-09-27) |
| Suite L2 in-repo | `tests/l2/` **8** suite · `npm run test:l2` · assertion floor + exit code ketat | **L2 PASS 313/0** (2026-09-27, pre & post-bump) |
...
| Suite terakhir fix-cetak | 21 PASS / 0 FAIL (L1 `bug_cetak_label3`) | **L1 PASS** |
| Suite terakhir viewport/mode | 19 PASS / 0 FAIL (4 kombinasi) | **L2 PASS** |
| Suite terakhir menu visual | 91 PASS / 0 FAIL (8 menu × viewer/editor × HP/desktop) | **L2 PASS** |
| Suite terakhir HP cetak fit | 64 PASS / 0 FAIL (4 viewport HP + desktop 1440) | **L2 PASS** |
| Suite terakhir sesi | 14 PASS / 0 FAIL (resume + idle 2 jam) | **L2 PASS** |
| Suite terakhir outbox app | 7 PASS / 0 FAIL (UI submit → pushOutbox → flushOutbox, cap 20, dedup) | **L2 PASS** |
| Suite terakhir escrow/xss | `xss_audit` 16/0 · `sec_classify` 39/0 + 6 ACCEPTED LIMITATION · `dup_check` 72/0 · `check_xe_ex` 36/0 | **L1 PASS** |
| Suite terakhir backend (eksekutabel) | `gs_trx_exec` 30/0 · `gs_auth_exec` 36/0 · `gs_stok_exec` 21/0 · `gs_aset_exec` 35/0 — **Code.gs dijalankan sungguhan** via `tests/tools/gs_harness.js` (fake Spreadsheet/Cache/Lock/Utilities) | **L1 PASS** |
...
| UI-15 | MED | Kolom MIN tabel Inventori: input kosong, edit ganda (bug revisi user 2026-09-25) | **DONE** v15.18 — kolom dihapus, atur via Detail Barang |
| UI-16 | MED | Garis antar baris di bawah tombol AKSI terpotong: `td` `display:flex` → `borderDiff` −4…−19,5px vs sel tabel | **DONE** v15.18 — bungkus `<span class="u-flex-cc-g5">`, CDP `borderDiff=0` |
| UI-05 | MED | 9 `<th th-sortable>` onclick tanpa role/tabindex (tak keyboard) | **DONE** v15.19 (F-06c) — 8 th → `<button type="button">` + CSS reset; CDP Enter 1x panggil sort |
| UI-06 | MED | Tombol ikon `&times;`/`▾`/`↻`/`✕`/🔦 tanpa accessible name | **DONE** v15.19 (F-06c) — 7 aria-label index + torch scanner.html |
| UI-07 | MED | 43 rule font <12px (min 8px) di label esensial | **DONE** v15.20 (F-06d) — 12 selector esensial 8/9/10px → `font-size:12px` |
...
| CETAK-01 | MED | Layar menu cetak terpotong di atas: `.subpage-bar` sticky `top:56/104` di dalam `#section-cetak` menimpa judul "Pilih Item untuk Dicetak" (bug user 2026-09-26) | **DONE** v15.26 — `#section-cetak .subpage-bar{position:relative;top:auto}` (specificity id menang; z-index 90 dipertahankan); probe overlap `false` PHONE/DESK + screenshot `fix1_PHONE/DESK_cetak_label.png` |
| CETAK-02 | MED | Download PDF label lambat: popup `window.open` + render html2pdf via CDN (async) + QR px 120 ≈ 49s/QR (akar kedua, benchmark Node; `qrcode.createDataURL` = O(px²)) (bug user 2026-09-26) | **DONE** v15.26 — `generateOutput` label+download → `downloadLabelPDF()` tanpa popup; jsPDF 2.5.1 vector lokal (`js/jspdf.min.js`); QR px 120→10; bukti: `buildLabelPDFDoc` 1 item 243–330ms, 4 item ~500ms, magic `%PDF-` |
| CETAK-03 | LOW–MED | QR/nama/spec label terlalu kecil (bug user 2026-09-26) | **DONE** v15.26 — `LABEL_TPL.qrMm` 15/13/20 → 27/17/22mm; font co/kode/nama/spec 3/6/5.5/4.5pt → 4/8/7.5/6pt; screenshot 3 template (hash unik) + qrw 102/64/83px = 27/17/22mm |
| HP-CETAK-01 | MED | Tampilan HP menu cetak label terpotong di kanan (430/375/360/320): `#section-cetak .container{margin:0 auto}` → auto-margin di sumbu cross mematikan align-stretch → lebar container = max-content ≈505px (dipaksa teks nowrap row list via `.cetak-item-nama/.cetak-item-sub`) → section `overflow:hidden` memotong (docSW==docCW, tanpa scrollbar); desktop 1440 ikut regresi sempit cont=655 sejak section jadi flex (bug user 2026-09-26) | **DONE** v15.27 — `width:100%` pada `#section-cetak .container` (fill s/d max-width 1100/1280/1520 lalu center); bukti: probe5 (NO-LIST→430, NO-NAV→505 = list satu-satunya pemaksa), probe6 (`min-width:0` tak berefek, `contain`→430), probe7 LIVE fix `okFit:true` ×4 viewport; suite baru `r2_hp_cetak_fit.js` 64/0 (container==vw, over[], ellipsis, btn wrap, bar tak clipping, desktop 1440 fill) |
| SESI-01 | HIGH | HP: "akun langsung logout" saat app dibuka lagi — boot migrasi memindahkan kredensial localStorage→sessionStorage lalu **menghapus** localStorage; PWA di-process-kill HP → sessionStorage hilang → `hasValidSession()` false → `showLoginGate()`; user minta batas waktu idle 2 jam auto-logout (bug user 2026-09-26) | **DONE** v15.27 — `writeSecureStorage` dual-write (session+local+touch `rdi_last_active`), `clearSecureStorage` hapus keduanya, migrasi **tanpa** hapus salinan (resume copy), `enforceSessionIdle()` dipanggil **sebelum** migrasi (urutan kritis: cek kredensial lokal juga — kalau sesudahnya, restore+touch membuat la selalu fresh & idle tak pernah terpicu) purge bila `now-la>SESSION_IDLE_MS` (2 jam): best-effort `viewerLogout` + hapus ketiga kredensial + `la` + reset global + `showLoginGate`; + `visibilitychange` re-check + heartbeat 60s touch; tradeoff token di localStorage ≤2 jam dibatasi TTL server 6 jam + denylist; suite baru `r2_session_idle.js` 14/0 (resume tab baru, la=30m tetap login, la=3h gate+purge) |
| OBS-01 | INFO | Observasi di luar scope (anti-creep, TIDAK di-fix): di 375×667 tinggi list cetak hanya ~80px (floor `min-height:80px`, scroll internal — by design); download kartu stok masih html2pdf CDN (hanya label pindah jsPDF) | report saja → T1 L4 / backlog jika user minta |
...
| TEST-11 | HIGH | 4 suite L1 (`check_xe_ex`, `dup_check`, `sec_classify`, `xss_audit`) **0 assertion** (cetak diagnostik salah klasifikasi sebagai suite) → 13% file L1 hijau tanpa coverage; `sec_classify` 6/12 verdict hardcoded + `process.exit(0)` | **DONE v15.28** — 4 suite jadi suite sungguhan (36/72/39/16 assert, verdict diturunkan dari kode) + `run-l1`/`run-l2` **assertion floor** (0 assert = FAIL, opt-out `RDI_ALLOW_ZERO_ASSERT=1`) + hitung stderr + anti double-count baris ringkasan |
| TEST-12 | HIGH | Runner hanya&& `exit===0 && no FAIL line` → suite diam/vakum tetap hijau | **DONE v15.28** — assertion floor + pesan `FAIL (NO ASSERTIONS EXECUTED)` |
| TEST-13 | HIGH | False-green L2: `r2_browser_suite` `exit(0)` saat CDP gagal; `r2_label_suite` tidak pernah set exit code untuk FAIL | **DONE v15.28** — `exit(2)` + `process.exitCode = fail?1:0` (+ `pass===0 → 2`) |
| TEST-14 | HIGH | Assertion outbox di L2 ditulis HARNESS (`localStorage.setItem('rdi_trx_outbox')`), bukan oleh app → lulus walau `js/outbox.js` rusak; cabang reconnect vakum (`auto:true` tanpa retry) | **DONE v15.28** — pakai `pushOutbox`/`readOutbox`/`flushOutbox` (API app) + suite baru `r2_outbox_app.js` 7/0 (UI `submitTransaksi()` → antrean → flush, cap 20, dedup) |
| TEST-15 | MED | `sim_authz_matrix.js:84` assertion vakum permanen (`… \|\| true`) | **DONE v15.28** — diganti cek nyata: semua `showStatus()` yang memakai `.message` ter-escape (35/0) |
| TEST-16 | MED | Suite L2 memakai `Get-Process chrome \| Stop-Process` → bisa menutup browser milik user; runner hanya kill 3 port | **DONE v15.28** — `r2_label_suite` hanya kill proses di CDP_PORT-nya; `run-l2` kill test chrome via `--remote-debugging-port` daftar (18 port) |
| TEST-17 | LOW | Assertion `r2_label_suite` cocokkan TEKS PERSIS body `ex()` → rapuh, merah palsu saat semantik escaper berubah | **DONE v15.28** — cek esensinya (escape `& < > "`) |
...
- [x] **Audit tampilan semua menu (user 2026-09-26)**: suite baru `r2_menu_visual_suite.js` **91/0** (8 menu master/cetak/alert/rak/history/aset/scanner/dashboard × HP/desktop × viewer/editor: elemen kunci ada & visible, 6 modal open/Escape-close, per-phase leak guard, screenshot ke Temp `menu-shots/`) · **2 temuan app di-fix**: (1) tabel Peringatan Stok tersembunyi di HP — CSS global `#section-master .table-wrap{display:none}` di media 640 ikut menelan `.table-wrap` di `#section-alert` → di-scope + `overflow-x:auto` (alert phone h 330→874, keyVisible ✓); (2) **MEDIUM** `#modal-filter`/`#modal-sort` berada di dalam `#section-master` → dibuka dari tab lain (jalur user `btn-open-filter-cetak` L1512) `w=0` tak terlihat → dipindah body-level (pure move, len identik 595103, 29/29 div; verifikasi 8 tab PASS + klik DOM cetak PASS) · **akar 1 bug test**: ekspresi Escape suite `(stmt;stmt)` = SyntaxError → leak modal-config palsu (`esc_diag_probe`: `__escSeen:0`, tanpa kurung `:1`) — BUKAN bug app · robustness suite: waitReady wajib `readyState==='complete'` (flake nav top=90 saat `interactive`), boot splash `!b` guard, `run-l2` timeout 180s→300s · gate L1 **519/0 (28 suite)** + L2 **228/0 (5 suite, 2× hijau pre/post-bump)** · v15.25 / sw `rdi-stok-v33` · sisa NOT TESTED: kamera device, print dialog, login form live, submit GAS live (L3/L4)  
- [x] **Fix 3 bug menu cetak (user 2026-09-26, approval eksplisit "PERBAIKI BUG INI")**: (1) **CETAK-01** layar terpotong — akar: `.subpage-bar` sticky `top:56/104` menimpa header di `#section-cetak` → sisip CSS `#section-cetak .subpage-bar{position:relative;top:auto}` (specificity 1,1,0 vs 0,1,0 & 0,2,1) · probe geo PHONE/DESK `overlap=false`, screenshot `fix1_*` judul+tombol terlihat; (2) **CETAK-02** PDF lambat — 2 akar: popup `window.open`+html2pdf CDN async & QR px 120 ≈ 49s/QR (benchmark Node `qr_px_measure.js`, `qrcode.createDataURL` = O(px²)) → `generateOutput` branch `label+download` → `downloadLabelPDF()` langsung tanpa popup; vendor jsPDF 2.5.1 (`js/jspdf.min.js`, `<script>` antara util & cetak, sw ASSETS +=), fungsi baru `labelPdfFilename`/`buildLabelPDFDoc` (vector A4 mm, dashed border, clamp nama≤2/spec≤1)/`downloadLabelPDF`; `scriptAction` label print-only (html2pdf cabang label dihapus; kartu tetap html2pdf = non-regression); QR px 120→10 · bukti 1 item 243–330ms / 4 item ~500ms, magic `%PDF-` · (3) **CETAK-03** QR/nama/spec kecil → `LABEL_TPL.qrMm` 15/13/20→27/17/22 + font 3/6/5.5/4.5→4/8/7.5/6pt · screenshot data-URL 3 template (`fix3_*`, hash unik, qrw 102/64/83px = 27/17/22mm; `document.write` frame basi → pindah `Page.navigate` data URL) · RED `bug_cetak_label3.js` 18F → GREEN **21/0**; `r2_label_suite.js` +2 rec (kartu html2pdf, label jsPDF vector tanpa popup, jsPDF loaded) 49→51P · gate L1 **540/0 (29 suite)** + L2 **230/0 (5 suite, pre & post-bump)** · v15.26 / sw `rdi-stok-v34` / pkg 15.26.0 · sisa NOT TESTED: unduhan PDF end-to-end via klik tombol (browser download), kamera device, print dialog, login form live, submit GAS live (L3/L4)  
- [x] **Fix 2 bug user (user 2026-09-26, approval eksplisit "perbaiki bug ini")**: (1) **HP-CETAK-01** tampilan HP menu cetak terpotong di kanan — akar TERBUKTI bertahap: probe3 (headRight 494-499 vs vw, modeBtns label +45..+160, docSW==docCW) → probe4 (sec.w=430, sec.sw=505, container.w=505) → probe5 (NO-LIST→container 430 = list pemaksa; NO-NAV→tetap 505; `min-width:0` injeksi TIDAK work) → probe6 (`contain:inline-size`/empty-rows→430 = konfirmasi max-content path; T1/T2 min-width:0 computed 0px tapi width tetap 504.609 karena auto cross-margin) → probe7 LIVE `width:100%` → `okFit:true` di 430/375/360/320 + desktop 1440 cont 655→1440 (fill ≤max-width 1520); fix 1 baris `#section-cetak .container{width:100%;…}` + komentar audit; RED `bug_hp_cetak_session.js` 19F → GREEN **20/0**; `cetak_bar_layout.js` regex rule disesuaikan (22/0); (2) **SESI-01** logout instan + idle 2 jam — akar: IIFE migrasi boot hapus salinan localStorage → process-kill HP = sesi lenyap; fix: dual-write resume copy + `SESSION_IDLE_MS` 2 jam + `enforceSessionIdle()` **sebelum** migrasi (urutan kritis — sesudah migrasi, restore+touch men-reset `rdi_last_active` sehingga idle tak pernah terpicu; sess check sertakan kredensial lokal) + purge (viewerLogout best-effort + clear ketiga key + la + reset global + gate) + visibilitychange + heartbeat 60s; suite baru L2 `r2_hp_cetak_fit.js` 64/0 + `r2_session_idle.js` 14/0 · gate L1 **560/0 (30 suite)** + L2 **308/0 (7 suite)** pre & post-bump · v15.27 / sw `rdi-stok-v35` / pkg 15.27.0 · sisa NOT TESTED: HP asli (L4), PWA process-kill nyata di device, login/logout live GAS (L3), visibilitas app di background 2 jam nyata  
- [x] **Tahap 1 — Integritas pengukuran (2026-09-27, atas permintaan " paws是为了 understand app secara detail")**: tujuan = "angka hijau harus berarti". (1) `run-l1.js:52` & `run-l2.js:76` **assertion floor** — suite 0-assert = FAIL (`RDI_ALLOW_ZERO_ASSERT=1` opt-out), + hitung dari stdout+stderr, + anti double-count baris ringkasan (`PASS \| x \| pass=N fail=M`); (2) 4 suite 0-assert → suite nyata: `check_xe_ex` **36** (mengeksekusi `js/util.js` via `new Function`; round-trip `xeJs` lewat literal JS; `xe(0)="0"`), `dup_check` **72** (fungsi wajib tunggal + baseline duplikasi disengaja + boundary FE/BE), `sec_classify` **39 + 6 ACCEPTED LIMITATION** (verdict dari kode; gate auth/token/idempotensi/lock/partial/formula-guard), `xss_audit` **16** (67 call site `showStatus` 0 kebocoran, 15 renderer wajib escaper, `document.write`=3, vendor lokal); (3) `r2_browser_suite` `exit(0)`→`exit(2)` saat CDP gagal; `r2_label_suite` set `process.exitCode`; (4) assertion outbox L2 diganti pakai API app `pushOutbox`/`readOutbox`/`flushOutbox` + **suite baru `r2_outbox_app.js` 7/0** (UI `submitTransaksi()` → offline → antrean → `flushOutbox`, cap 20, dedup requestId); (5) `sim_authz_matrix.js:84` `\|\| true` → cek nyata; (6) hentikan `Get-Process chrome` (tidak menutup browser user) — `run-l2` kill via daftar `--remote-debugging-port`; (7) **ESC-01** fix `xe()`/`ex()`/`xeJs()` `String(s\|\|'')`→`String(s==null?'':s)` (nilai `0` tak jadi kosong; terkonfirmasi runtime: mock `qty:0` → sel stok kosong sebelum fix). · gate **L1 560→722/0 (30 suite)** + **L2 308→313/0 (7→8 suite)** pre & post-bump · v15.28 / sw `rdi-stok-v36` / pkg 15.28.0 · 6 temuan keamanan tetap **ACCEPTED LIMITATION** (BE-01 KDF, BE-02 rate limit editor key, BE-03 audit log, BE-05 formula guard Aset, BE-07 header kredensial, CL-03 CDN tanpa SRI, OFF-01 cap outbox) — jadi target Tahap 3
- [x] **Tahap 2 — Harness `Code.gs` eksekutabel (2026-09-27)**: `tests/tools/gs_harness.js` memuat `Code.gs` (202 KB / 4.041 baris) ke `vm` context dengan stub SpreadsheetApp (sheet in-memory, **range live-view** — versi awal mengembalikan copy sehingga updateSaldo "sukses" tanpa mengubah data), CacheService (TTL deterministik), LockService, Utilities (**computeDigest harus mengembalikan array-of-byte seperti GAS**, bukan Buffer — kalau tidak `hashPasswordHex_` menghasilkan hex sampah), PropertiesService, ContentService. Empat suite: `gs_trx_exec` **30** (MASUK/KELUAR mutasi sheet, floor per-rak, idempotensi `requestId`+body-signature, item arsip, 5 validasi, safeCell_, partial saat sinkron saldo gagal), `gs_auth_exec` **36** (login benar/salah, anti-enumerasi, akun mati, password default, token HMAC, denylist, pv-revocation, rate limit 5/15 mnt, constant-time, hash shape, token tak memuat password), `gs_stok_exec` **21** (incremental == rebuild == replay independen, floor per-rak, `getItemById`, kontrak `qty` sebagai **String** — regression lock ESC-01, `addItem` stok awal), `gs_aset_exec` **35** (state machine 7 activity lengkap, transisi invalid, KARAT/SCRAP, eligible units, `computeStatusKondisi`, dashboard). · KDF 100.000 iterasi di-override ke 1.000 untuk runtime test (83s→4s; nilai produksi tetap di-assert) · backend: **0 → 122 assert eksekutabel** · gate **L1 722→844/0 (34 suite)**
- [ ] **Tahap 3 — Fix 6 ACCEPTED LIMITATION backend** (OFF-01 outbox, BE-01 KDF short-circuit, BE-02 rate limit editor key, BE-04 baca-vs-recalc, BE-05 formula guard Aset, BE-07 jalur kredensial) — **butuh approval deploy `Code.gs`** (AGENTS.md §C); sekarang bisa dibuktikan dengan harness sebelum/sesudah
- [ ] F-06+ (P3 antrian)  
...
| 2026-09-25 | Revisi bug (user): UI-15 hapus kolom MIN tabel Inventori (atur di Detail Barang; `var minCell`/td/th min + `sortByColumn('minStock')` + colspan 11→10 ×2); UI-16 garis antar baris di bawah tombol AKSI tak sejajar — td `class="c u-flex-cc-g5"` (`display:flex` keluar table-cell box) → td `c` + `<span class="u-flex-cc-g5">` wrap; v15.18, sw `rdi-stok-v26` | approval langsung user (revisi bug); bukti CDP `meas_aksi.js` borderDiff −4…−19,5px → 0 + screenshot `shot_inv.js`; RED `ui07_master_min_aksi.js` 3P/9F → GREEN 12/0 → gate L1 414/0 (21 suite) + L2 81/0 | PLAN.md v1.16 |
...
| 2026-09-26 | Audit tampilan semua menu (user: cek semua tampilan semua menu): 2 temuan app = BUG alert mobile (CSS global `#section-master .table-wrap` menelan alert di HP → scoped media 640 + overflow-x) + BUG filter/sort invisible lintas tab (modal di dalam `#section-master`, `w=0` dari tab cetak → dipindah body-level); sisanya 3 temuan = bug test (ekspresi Escape SyntaxError → leak modal-config palsu; boot splash race; runner timeout 180s) | bukti: `r2_menu_visual_suite` 91/0, `loc_probe` 8-tab PASS, `esc_diag_probe` SyntaxError→`__escSeen:1`, L1 519/0 + L2 228/0 (2x); bump v15.25 / sw `rdi-stok-v33` | PLAN.md v1.25 |
| 2026-09-26 | Fix 3 bug menu cetak (user: PERBAIKI BUG INI): (1) subpage-bar sticky menimpa header → `position:relative` in-flow; (2) PDF lambat → tanpa popup + jsPDF vector lokal + QR px 120→10 (akar: O(px²) 49s/QR); (3) QR/nama/spec kecil → qrMm 27/17/22 + font 4/8/7.5/6pt; kartu stok download tetap html2pdf (di luar scope) | bukti: RED→GREEN `bug_cetak_label3` 18F→21/0, `r2_label_suite` 51/0, probe PDF 243–330ms/item, screenshot `fix1_*`+`fix3_*` (hash unik), L1 540/0 + L2 230/0 (2x pre/post-bump); bump v15.26 / sw `rdi-stok-v34` | PLAN.md v1.26 |
...
| 1.25 | 2026-09-26 | Audit tampilan semua menu: suite baru `r2_menu_visual_suite.js` (L2 5 suite, 228/0) + 2 fix app (alert mobile scoped CSS, modal filter/sort pindah body-level) + 3 fix test (Escape SyntaxError, boot `readyState` guard, runner timeout); v15.25 / sw `rdi-stok-v33`; §1.2, §10, §11, §13 |

| 1.26 | 2026-09-26 | Fix 3 bug menu cetak (CETAK-01 subpage-bar in-flow, CETAK-02 download PDF tanpa popup via jsPDF vector + QR px 10, CETAK-03 qrMm 27/17/22 + font label); suite baru `bug_cetak_label3.js` (L1 29 suite, 540/0) + `r2_label_suite` 49→51P; gate L2 230/0 (2x); v15.26 / sw `rdi-stok-v34` / pkg 15.26.0; §1.2, §1.4, §10, §11, §13 |

| 1.27 | 2026-09-26 | Fix 2 bug user: HP-CETAK-01 `width:100%` container cetak (probe3–7 + LIVE okFit ×4) + SESI-01 resume dual-write & auto-logout idle 2 jam `enforceSessionIdle()` sebelum migrasi; suite baru `bug_hp_cetak_session.js` (L1 30 suite, 560/0), `r2_hp_cetak_fit.js` + `r2_session_idle.js` (L2 7 suite, 308/0; 2x pre/post-bump); `cetak_bar_layout` regex adaptasi; v15.27 / sw `rdi-stok-v35` / pkg 15.27.0; §1.2, §1.4 (HP-CETAK-01, SESI-01, OBS-01), §10, §11, §13 |

| 1.28 | 2026-09-27 | Tahap 1 integritas pengukuran: **assertion floor** L1+L2; 4 suite 0-assert → suite nyata (`check_xe_ex` 36, `dup_check` 72, `sec_classify` 39+6 AL, `xss_audit` 16); false-green L2 ditutup; outbox L2 via API app + suite baru `r2_outbox_app.js` 7/0; ESC-01 fix `xe()` 0→"0"; L1 **722/0** (30 suite) + L2 **313/0** (8 suite) pre & post-bump; v15.28 / sw `rdi-stok-v36` / pkg 15.28.0; §1.2, §1.4 (ESC-01, TEST-11…17), §10, §11, §13 |
