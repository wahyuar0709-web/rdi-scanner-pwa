...
| Version | 1.26 |
| Date | 2026-09-26 |
| Repo HEAD | `9da3958` (Fix 3 bug cetak label + bump v15.26) |
| App version | `v15.26` / SW `rdi-stok-v34` / GAS deployment `@41` |
...
|------|-------|--------------|
| Git HEAD | `9da3958` (Fix 3 bug cetak label + bump v15.26) | — |
| Working tree | clean | — |
| `APP_VERSION` | `v15.26` (source of truth + title/apple/css/js/topbar sinkron) | L1 PASS |
| `APP_BUILD_DATE` | `2026-09-26` | L1 |
| SW `CACHE` | `rdi-stok-v34` (`sw.js:1`) + precache `./js/util.js` + `./js/cetak.js` + `./js/outbox.js` + `./js/format.js` + `./js/jspdf.min.js` | L1+L2 |
| `index.html` lines | ~4316 (Revisi Cetak: shell flex `#section-cetak` + bar in-flow; audit-tampilan: CSS alert scoped `@media 640` + `#modal-filter`/`#modal-sort` pindah body-level; F-06f: h1 login + ::after 44px + hash deep link + prefers-color-scheme; F-06e: 4 fallback `100dvh` + `role="dialog"` ×2; BUG-01: +8 export; fix-3-bug cetak: `#section-cetak .subpage-bar` in-flow + `<script jspdf.min.js>` + `generateOutput` label-download tanpa popup) | L1 |
| Suite L1 in-repo | `tests/l1/` **29** suite · `npm run test:l1` | **L1 PASS 540/0** (2026-09-26) |
| Suite L2 in-repo | `tests/l2/` 5 suite · `npm run test:l2` | **L2 PASS 230/0** (2026-09-26) |
