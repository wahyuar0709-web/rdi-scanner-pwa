...
// ============================================================
//  v5.22 — EXPORT EXCEL LENGKAP (format "kartu stok" lama, per-batch,
//  dikelompokkan per Kategori -- meniru struktur file lama
//  "Tools & Spare Part Inventory ....xlsx": tiap SHEET = 1 kategori,
//  tiap BARIS = 1 batch kedatangan (bukan 1 baris per item), dgn
//  kolom tanggal berisi Qty KELUAR pada tanggal itu (BUKAN saldo),
//  dan kolom terakhir "Stok Akhir" = sisa stok batch itu.
//
//  MASALAH YANG DIPECAHKAN: skema aktif (v5.0+) TIDAK menyimpan dari
//  batch mana suatu transaksi KELUAR diambil -- Transaksi_Log cuma
//  catat saldo AGREGAT per item. Supaya bisa direkonstruksi jadi
//  "kartu stok per kedatangan" seperti file lama, fungsi ini
//  MENYIMULASIKAN alokasi FIFO: tiap transaksi KELUAR dianggap
//  memotong stok dari batch MASUK yang paling TUA dulu (yang
//  remaining-nya belum 0), baru lanjut ke batch berikutnya kalau
//  batch itu habis. ASUMSI, bukan fakta tercatat -- kalau dulu
//  petugas gudang tidak selalu ambil barang FIFO, hasil rekonstruksi
//  ini bisa beda dari kondisi fisik aslinya. Simulasi selalu jalan
//  atas SELURUH riwayat Transaksi_Log (supaya Stok Akhir tiap batch
//  akurat) -- dateFrom/dateTo HANYA membatasi kolom tanggal mana yang
//  ditampilkan di sheet, tidak membatasi data yang disimulasikan.
//
//  Pengelompokan sheet: per NILAI Kategori di Master_Item (dinamis --
//  apa pun yang ada di data, bukan 4 nama sheet lama yang di-hardcode,
//  karena Master_Kategori sekarang bisa beda isi dari file lama).
//  Item tanpa Kategori diisi masuk ke grup "Lainnya".
//
//  Kolom User & BC/Non BC SELALU ditampilkan di semua sheet kategori
//  (penyederhanaan dari file lama yang dulu cuma punya kolom ini di
//  sheet Consumable & Indirect) -- karena field ini sekarang melekat
//  di SETIAP item apa pun kategorinya, bukan cuma sebagian.
//
//  Params:
//   dateFrom, dateTo : string 'yyyy-MM-dd', OPSIONAL. Kosong keduanya
//                      berarti semua tanggal KELUAR ditampilkan.
// ============================================================
function getExportData(dateFrom, dateTo) {
  try {
    ...
    // --- 1) Info identitas item (nama, spec, user, bc, unit, kategori) ---
    var itemInfo = {}; // id -> {nama, spec, user, bc, unit, kategori}
    if (master && master.getLastRow() >= 2) {
      ...
      mData.forEach(function(r){
        ...
        if (!id) return;
        itemInfo[id] = {
          nama: String(r[COL_MASTER.NAMA]||''),
          spec: String(r[COL_MASTER.SPEC]||''),
          user: String(r[COL_MASTER.USER]||''),
          bc: String(r[COL_MASTER.BC]||''),
          unit: String(r[COL_MASTER.UNIT]||''),
          kategori: String(r[COL_MASTER.KATEGORI]||'').trim() || 'Lainnya'
        };
      });
    }

    // --- 2) Kumpulkan SELURUH transaksi per item, kronologis (untuk simulasi FIFO) ---
    var txByItem = {}; // id -> [{ts, jenis, qty, rak, vendor, noRef}]
    if (trx && trx.getLastRow() >= 2) {
      ...
      tData.forEach(function(row){
        ...
        if (!id) return;
        var tsDate = row[COL_TRX.TIMESTAMP];
        var ts = tsDate instanceof Date ? tsDate.getTime() : 0;
        if (!txByItem[id]) txByItem[id] = [];
        txByItem[id].push({
          ts: ts,
          jenis: String(row[COL_TRX.JENIS]||'').toUpperCase(),
          qty: parseFloat(row[COL_TRX.QTY])||0,
          rak: String(row[COL_TRX.RAK]||''),
          vendor: String(row[COL_TRX.VENDOR]||''),
          noRef: String(row[COL_TRX.NO_REF]||'')
        });
      });
    }

    // --- 3) Simulasi FIFO per item: pecah jadi batch (tiap MASUK = 1 batch),
    //         tiap KELUAR memotong batch tertua yang masih ada sisa dulu ---
    var sheetsMap   = {}; // kategori -> array of batch rows
    var dateColsMap = {}; // kategori -> { 'dd/MM/yyyy': true } (hanya dalam rentang filter)

    Object.keys(txByItem).forEach(function(id){
      var info = itemInfo[id] || { nama:'', spec:'', user:'', bc:'', unit:'', kategori:'Lainnya' };
      var kategori = info.kategori || 'Lainnya';
      var txs = txByItem[id].slice().sort(function(a,b){ return a.ts - b.ts; });

      var batches = []; // {receivedTs, vendor, noRef, rak, qtyIn, remaining, keluarByDate:{}}
      txs.forEach(function(t){
        if (t.jenis === 'MASUK') {
          batches.push({
            receivedTs: t.ts, vendor: t.vendor, noRef: t.noRef, rak: t.rak,
            qtyIn: t.qty, remaining: t.qty, keluarByDate: {}
          });
        } else if (t.jenis === 'KELUAR') {
          var sisaKeluar = t.qty;
          var dateKey = fmtDate(new Date(t.ts));
          for (var bi = 0; bi < batches.length && sisaKeluar > 0; bi++) {
            var b = batches[bi];
            if (b.remaining <= 0) continue; // batch ini sudah habis, lanjut ke batch berikutnya
            var take = Math.min(b.remaining, sisaKeluar);
            b.remaining -= take;
            sisaKeluar -= take;
            b.keluarByDate[dateKey] = (b.keluarByDate[dateKey]||0) + take;
            if ((!fromTs || t.ts >= fromTs) && (!toTs || t.ts <= toTs)) {
              if (!dateColsMap[kategori]) dateColsMap[kategori] = {};
              dateColsMap[kategori][dateKey] = true;
            }
          }
          // Kalau sisaKeluar masih >0 di sini berarti data historis pernah nyeleneh
          // (KELUAR lebih besar dari total MASUK yg tercatat sampai titik itu) --
          // dibiarkan (tidak dialokasikan ke batch mana pun) daripada bikin batch
          // "hantu" -- kasus ini seharusnya sudah dicegah validasi postTransaksi().
        }
      });

      if (!sheetsMap[kategori]) sheetsMap[kategori] = [];
      batches.forEach(function(b){
        sheetsMap[kategori].push({
          idItem: id, nama: info.nama, spec: info.spec, user: info.user, bc: info.bc,
          vendor: b.vendor, noRef: b.noRef, receivedTs: b.receivedTs,
          rak: b.rak, qtyIn: b.qtyIn, unit: info.unit,
          keluarByDate: b.keluarByDate, stokAkhir: b.remaining
        });
      });
    });

    // --- 4) Susun output per kategori: urut baris per item lalu per tanggal
    //         kedatangan, urut kolom tanggal kronologis ---
    var sheetsOut = [];
    Object.keys(sheetsMap).sort().forEach(function(kategori){
      var rows = sheetsMap[kategori].sort(function(a,b){
        if (a.idItem !== b.idItem) return a.idItem < b.idItem ? -1 : 1;
        return a.receivedTs - b.receivedTs;
      });
      var dateCols = Object.keys(dateColsMap[kategori] || {}).sort(function(a,b){
        function toTsLocal(d){ var p=d.split('/'); return new Date(parseInt(p[2],10), parseInt(p[1],10)-1, parseInt(p[0],10)).getTime(); }
        return toTsLocal(a) - toTsLocal(b);
      });
      sheetsOut.push({
        kategori: kategori,
        ...
        rows: rows.map(function(r, idx){
          return {
            no: idx+1, idItem: r.idItem, nama: r.nama, spec: r.spec,
            user: r.user, bc: r.bc, vendor: r.vendor, noReferensi: r.noRef,
            receivedDate: fmtDate(new Date(r.receivedTs)), rak: r.rak,
            qtyIn: r.qtyIn, unit: r.unit,
            keluarPerTanggal: r.keluarByDate, // { 'dd/MM/yyyy': qty } -- hanya tanggal yg ada KELUAR utk batch ini
    ...
    return {
      status: 'ok',
      sheets: sheetsOut, // array of { kategori, dateColumns, rows }
