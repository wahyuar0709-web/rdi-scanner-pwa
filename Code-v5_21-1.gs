...
//
//  CHANGELOG v5.21 (Fix PERFORMA — laporan "transaksi lelet", audit lanjutan):
//   - postTransaksi() dulu panggil getItemById() yang serba-guna -- termasuk
//     getLastVendorRefMap(), yaitu scan SELURUH Transaksi_Log (via cache 45dtk,
//     tapi tetap forEach di JS thread) HANYA untuk isi field vendor/noReferensi/
//     tglMasukTerakhir yang TIDAK PERNAH dipakai postTransaksi. Makin banyak
//     transaksi menumpuk sepanjang waktu, makin lambat SETIAP kali submit
//     transaksi baru -- padahal item ini yang ditransaksikan sama sekali tidak
//     butuh info itu. Sekarang postTransaksi pakai getItemForTransaksi_() baru:
//     versi ringan tanpa getLastVendorRefMap() sama sekali.
//   - Selain itu, Stok_Saldo & Stok_Per_Rak dulu dibaca PENUH 2x per transaksi:
//     sekali di getItemById (buat validasi saldo sebelum), sekali lagi di
//     updateSaldo()/updateRakSaldo() (buat cari baris yang mau diupdate) --
//     padahal datanya identik, sheet belum berubah di antara kedua baca itu.
//     getItemForTransaksi_() sekarang baca KEDUANYA cuma 1x, baris mentahnya
//     dikirim ke updateSaldo()/updateRakSaldo() lewat parameter baru
//     `existingRows` (opsional -- kalau tidak dikirim, keduanya fallback baca
//     sendiri seperti sebelumnya, jadi caller lain/lama tidak terpengaruh).
//   - getItemById() (dipakai action 'getItem' utk tampilan detail) TIDAK
//     diubah -- itu memang butuh vendor/tglMasuk utk ditampilkan ke user.
//   - Ringkasnya: round-trip Sheets API per 1x submit transaksi turun dari
//     ~7-9x jadi ~4-5x, dan yang paling penting: hilang 1 komponen yang
//     tadinya tumbuh sebanding dgn TOTAL transaksi sepanjang masa (bukan lagi
//     O(total transaksi) tiap submit, cukup O(jumlah item/rak)).
//
...
// ============================================================
// ============================================================
//  GET ITEM FOR TRANSAKSI (baru — v5.21, fix performa)
//  Versi RINGAN dari getItemById(), khusus dipakai postTransaksi().
//  Bedanya:
//   1. TIDAK panggil getLastVendorRefMap() -- info vendor/noReferensi/
//      tglMasukTerakhir tidak pernah dipakai postTransaksi, tapi dulu ikut
//      dihitung lewat getItemById() (dan itu scan seluruh Transaksi_Log).
//   2. Baris mentah Stok_Saldo & Stok_Per_Rak yang dibaca di sini DIKEMBALIKAN
//      (saldoRows, rakRows) supaya updateSaldo()/updateRakSaldo() bisa pakai
//      ulang tanpa baca sheet yang sama lagi.
//  getItemById() TIDAK dihapus/diubah -- tetap dipakai action 'getItem' yang
//  memang perlu vendor/tglMasuk utk ditampilkan ke user.
// ============================================================
function getItemForTransaksi_(itemId) {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var masterSheet = ss.getSheetByName(SHEET_MASTER);
    if (!masterSheet) return { status:'error', message:'Sheet Master_Item tidak ditemukan' };

    itemId = String(itemId||'').trim().toUpperCase();
    if (!itemId) return { status:'error', message:'ID kosong' };

    var masterLastRow = masterSheet.getLastRow();
    if (masterLastRow < 2) return { status:'error', message:'Data kosong' };

    var masterData = masterSheet.getRange(2, 1, masterLastRow-1, 10).getValues();
    var masterRow = null;
    for (var i = 0; i < masterData.length; i++) {
      if (String(masterData[i][COL_MASTER.ID]||'').trim().toUpperCase() === itemId) { masterRow = masterData[i]; break; }
    }
    if (!masterRow) return { status:'error', message:'Item '+itemId+' tidak ditemukan' };

    var saldoSheet = ss.getSheetByName(SHEET_SALDO);
    var saldoRows = null;
    var saldoAkhir = 0; // dibaca LANGSUNG dari kolom SALDO_AKHIR (bukan dihitung ulang
                         // masuk-keluar) -- samakan sumber kebenaran dgn getSaldoFullMap(),
                         // jaga-jaga ada penyesuaian manual di sheet yg bikin nilainya
                         // sedikit beda dari masuk-keluar murni.
    if (saldoSheet) {
      var saldoLastRow = saldoSheet.getLastRow();
      if (saldoLastRow >= 2) {
        saldoRows = saldoSheet.getRange(2,1,saldoLastRow-1,6).getValues();
        var sIdx = findRowIndex(saldoRows, [[COL_SALDO.ID, itemId]]);
        if (sIdx >= 0) {
          saldoAkhir = parseFloat(saldoRows[sIdx][COL_SALDO.SALDO_AKHIR])||0;
        }
      }
    }

    var rakSheet = ss.getSheetByName(SHEET_RAK_SALDO);
    var rakRows = null;
    var rakBreakdown = [];
    if (rakSheet) {
      var rakLastRow = rakSheet.getLastRow();
      if (rakLastRow >= 2) {
        rakRows = rakSheet.getRange(2,1,rakLastRow-1,3).getValues();
        rakRows.forEach(function(row){
          if (String(row[COL_RAK_SALDO.ID]||'').trim().toUpperCase()===itemId && parseFloat(row[COL_RAK_SALDO.QTY])!==0) {
            rakBreakdown.push({ rak:String(row[COL_RAK_SALDO.RAK]||''), qty:parseFloat(row[COL_RAK_SALDO.QTY])||0 });
          }
        });
      }
    }
    ...
    return {
      ...
      item:{
        id: itemId,
        nama: String(masterRow[COL_MASTER.NAMA]||''),
        spec: String(masterRow[COL_MASTER.SPEC]||''),
        unit: String(masterRow[COL_MASTER.UNIT]||''),
        status: String(masterRow[COL_MASTER.STATUS]||'').trim() || 'Aktif',
        saldo: saldoAkhir,
        rakBreakdown: rakBreakdown
      },
      saldoRows: saldoRows,
      rakRows: rakRows
    };
  } catch(err) {
    return { status:'error', message:'getItemForTransaksi_: ' + err.message };
  }
}
...
function postTransaksi(params) {
  ...
  try {
    ...
    // PERF v5.21: getItemForTransaksi_ (bukan getItemById) -- tidak scan Transaksi_Log
    // buat vendor/tglMasuk yang tidak dipakai di sini, dan baris Stok_Saldo/Stok_Per_Rak
    // yang dibaca di dalamnya dipakai ulang di bawah (lihat itemResult.saldoRows/rakRows).
    var itemResult = getItemForTransaksi_(itemId);
    ...
    var saldoSyncError = '';
    try {
      // PERF v5.21: kirim saldoRows yang sudah dibaca getItemForTransaksi_ -- updateSaldo
      // tidak perlu getRange lagi ke Stok_Saldo (sheet belum berubah sejak dibaca).
      updateSaldo(itemId, item.nama, item.unit, jenis, qty, itemResult.saldoRows);
    } catch(e) {
      saldoSyncOk = false;
      saldoSyncError += e.message + '. ';
    }
    try {
      // PERF v5.21: sama seperti di atas, pakai ulang rakRows -- tidak baca Stok_Per_Rak lagi.
      updateRakSaldo(itemId, rak, jenis, qty, itemResult.rakRows);
    } catch(e) {
      saldoSyncOk = false;
      saldoSyncError += e.message + '. ';
    }
...
// ============================================================
function updateSaldo(itemId, nama, unit, jenis, qty, existingRows) {
  try {
    var ss    = SpreadsheetApp.getActiveSpreadsheet();
    var saldo = ss.getSheetByName(SHEET_SALDO);
    if (!saldo) return;
    ...
    // PERF v5.21: kalau caller (postTransaksi, lewat getItemForTransaksi_) sudah baca
    // sheet ini duluan, baris mentahnya dikirim lewat existingRows -- getRange TIDAK
    // dipanggil lagi di sini. Caller lain yang tidak kirim existingRows tetap fallback
    // baca sendiri seperti sebelumnya (perilaku lama utuh, tidak breaking).
    if (existingRows || lastRow >= 2) {
      var existing = existingRows || saldo.getRange(2,1,lastRow-1,6).getValues();
...
// ============================================================
function updateRakSaldo(itemId, rak, jenis, qty, existingRows) {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sh = ss.getSheetByName(SHEET_RAK_SALDO);
    if (!sh) return;
    ...
    // PERF v5.21: existingRows (dari getItemForTransaksi_ via postTransaksi) dipakai ulang
    // kalau ada -- tidak getRange lagi. Fallback baca sendiri kalau caller tidak kirim.
    if (existingRows || lastRow >= 2) {
      var existing = existingRows || sh.getRange(2,1,lastRow-1,3).getValues();
